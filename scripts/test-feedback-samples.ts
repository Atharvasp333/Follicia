    ...
    try {
      ...
      if (process.env.GEMINI_API_KEY) {
        try {
          ...
          console.log(
            `  ✓ AI Analysis: ${analysis.aiCategory} | ${analysis.sentiment} | Urgency: ${analysis.urgencyScore}/10`
          );
        } catch (error) {
      ...
      const feedback = await prisma.feedback.create({
        data: {
          ...
          aiCategory: analysis?.aiCategory || "PRODUCT",
          sentiment: analysis?.sentiment || "NEUTRAL",
          aiSummary: analysis?.aiSummary || sample.rawComment.substring(0, 100),
